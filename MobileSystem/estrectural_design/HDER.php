	...
	while ($rows = odbc_fetch_object($consultar_nombre_empresa)) { 
		...
		//CONSULTAMOS EL TIPO DE EMPRESA - Distribuidor o Maestro -
					if ($tipo=='Distribuidor') {
						...
						$consulta_empresas_hijas = odbc_exec ($conexion,"SELECT * FROM GG_MULTI_USUARIOS WHERE usuario_padre_secundario='".$_SESSION['MM_Username']."'");
							while ($rows2 = odbc_fetch_object($consulta_empresas_hijas)) {
		
							echo '<option>'.$select_empresa=$rows2->usuario_hijo.'</option>'; 
