...
$query_id_vehicle=odbc_exec($conexion,"SELECT * FROM E_INVENTARIO_GENERAL WHERE nombre_usuario='".$_SESSION['MM_Username']."'");
					...
					echo'</select>
							...
							<td>&nbsp;</td>
							<td align="center" valign="middle" bgcolor="#000"><a href="#seguimiento" style="color:#fff;">Ver Historial</a></td>
							<td>&nbsp;</td>
...
$consulta=odbc_exec($conexion,"SELECT * FROM ZLECTURAS WHERE id_servidor='".$nuestro_id."' AND hora_servidor BETWEEN '".$cadena_fecha_inicio."' AND '".$cadena_fecha_temno."' ORDER BY id ASC ");
...
var flightPath = new google.maps.Polyline({
  ...
  for(var i in flightPlanCoordinates){
	...
	$consulta_ultima_posicion_para_marcador=odbc_exec($conexion,"SELECT * FROM ZLECTURAS WHERE id_servidor='".$nuestro_id."' AND hora_servidor BETWEEN '".$cadena_fecha_inicio."' AND '".$cadena_fecha_temno."' ORDER BY id ASC ");
	...
	$consulta_ultima_posicion_para_marcadorunico=odbc_exec($conexion,"SELECT * FROM ZLECTURAS WHERE id_servidor='".$nuestro_id."' AND hora_servidor BETWEEN '".$rango_unico_recibido."' AND '".$rango_unico_recibido."' ORDER BY id ASC ");
...
$consulta2=odbc_exec($conexion,"SELECT top 500 * FROM ZLECTURAS WHERE id_servidor='".$nuestro_id."' AND hora_servidor BETWEEN '".$cadena_fecha_inicio."' AND '".$cadena_fecha_temno."' ORDER BY id DESC  ");
...
<h4 style="float:left; margin:0; margin-left:10%;">Historial del veh&iacute;culo : "'.$filtro_vehiculo.'"</h4>
				...
				<form action="panel.php" method="post" style="float:right; margin-right:10%;">
					<input name="id_vehiculo" type="hidden" value="'.$id_servidor.'">
					<input name="nombre_vehiculo" type="hidden" value="'.$nombre_vehiculo.'">
					<input name="filtro_vehiculo" type="hidden" value="'.$filtro_vehiculo.'">
					...
					<input name="latlng" type="hidden" value="'.$lat.','.$lng.'" />
					<input name="lat" type="hidden" value="'.$lat.'" />
					<input name="lng" type="hidden" value="'.$lng.'" />
					<input name="consulta" type="hidden" value="midireccionenmapa" />
					<input id="submit" type="submit" value="Detalles del Recorrido" style="width:200px; background:#000; border:none; color:#fff; height:28px; cursor:pointer; text-decoration:underline;">
				</form>
</div>
';
if (isset($lat)) 
if (isset($lng)) 
echo '
		<table width="80%" border="1" bordercolor="fff" cellpadding="5" cellspacing="5" align="center"  style="font-size:75%;">
...
while ($datos=odbc_fetch_object($consulta2)) 
			...
			<tr>
				...
				<td>'.$odometro_sin_centecimas.' kms</td>
				<td>'.$bateria.'</td>

				<td>';//ALERTAS------------------
				$consulta_mi_alerta=odbc_exec($conexion,"SELECT TOP 1 * FROM ZALERTAS WHERE  id_servidor='".$id_servidor."' ORDER BY id DESC");
						
				
						while ($mi_alerta=odbc_fetch_object($consulta_mi_alerta))
						{
							$tipo_cabecera=$mi_alerta->HDR;
							$tipo_alerta=$mi_alerta->EVT_ID;
							
								if($tipo_alerta==1) {$dato='Exeso de velocidad';}
								if($tipo_alerta==2) {$dato='Velocidad normal';}
								if($tipo_alerta==10) {$dato='10';}
								if($tipo_alerta==14) {$dato='Bater&iacute;a fuera de carga';}
								if($tipo_alerta==15) {$dato='Golpe modo estacionado';}
								if($tipo_alerta==16) {$dato='Colisi&oacute;n modo encendido';}
								if($tipo_alerta==33) {$dato='';}
								if($tipo_alerta==34) {$dato='';}
								if($tipo_alerta==40) {$dato='Energ&iacute;a principal conectada';}
								if($tipo_alerta==41) {$dato='Energ&iacute;a principal desconectada';}
								if($tipo_alerta==44) {$dato='Se conecto el backup gps';}
								if($tipo_alerta==45) {$dato='Se desconecto el backup gps';}
								if($tipo_alerta==46) {$dato='Aceleraci&oacute;n subita';}
								if($tipo_alerta==47) {$dato='Frenado brusco';}
								if($tipo_alerta==48) {$dato='Vuelta cerrada';}
								if($tipo_alerta==49) {$dato='Exeso de velocidad';}
								if($tipo_alerta==68) {$dato='Tiempo de encendido exedido';}
								if($tipo_alerta==69) {$dato='Veh&iacute;culo en movimiento';}
								
							
							
							$id_cazado_alertas=$mi_alerta->id_servidor;
							$nombre_vehiculo=$mi_alerta->nombre_vehiculo;
				...
				<td>';//EVENTOS------------------
				$consulta_mi_evento=odbc_exec($conexion,"SELECT TOP 1 * FROM ZEVENTOS WHERE id_servidor='".$id_servidor."' ORDER BY id DESC");
						
				
						
				
						while ($mi_evento=odbc_fetch_object($consulta_mi_evento))
						{
							
							$tipo_evento=$mi_evento->EVT_ID;
								
								if($tipo_evento==1) {$dato2='Conectado E-1';}
								if($tipo_evento==2) {$dato2='Desconectado E-1';}
								if($tipo_evento==3) {$dato2='Conectado E-2';}
								if($tipo_evento==4) {$dato2='Desconectado E-2';}
								if($tipo_evento==5) {$dato2='Conectado E-3';}
								if($tipo_evento==6) {$dato2='Desconectado E-3';}
								
							$id_cazado_alertas=$mi_evento->id_servidor;
							$nombre_vehiculo=$mi_evento->nombre_vehiculo;
				...
				<td>
					<form name="" action="panel.php" method="post">
							    <input name="unico" type="hidden" value="">
								<input name="filtro_vehiculo" type="hidden" value="'.$filtro_vehiculo.'">
								<input name="dia" type="hidden" value="'.$dia.'">
								<input name="mes" type="hidden" value="'.$mes.'">
								<input name="anio" type="hidden" value="'.$anio.'">
								<input name="rango" type="hidden" value="'.$rango.'">
								<input name="rango_unico" type="hidden" value="'.$ultima_posicion.'">
							    <input name="confirma" type="hidden" value="si" />
								<input type="hidden" id="latlng" type="text" value="'.$lat.','.$lng.'">
                                <input name="consulta" type="hidden" value="mirecorridoenmapa" />
					   			<input name="unico" type="submit" id="submit" value="Ubicaci&oacute;n" class="btn_ubicacion_recorrido">
					 </form>
				</td>
