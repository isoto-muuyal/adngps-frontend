		...
		?>
                 ...
                 <div id="funcion_opacidad">
					...
					<form action="panel.php" method="post">
						...
						<input name="Boofers" type="hidden" value="'.$Boofer.'" />
                        <input name="contrasenias" type="hidden" value="'.$contrasenia.'" />
                        <input name="nombre_empresas" type="hidden" value="'.$nombre_empresa.'" />
						<input name="nombre_completos" type="hidden" value="'.$nombre_completo.'" />
                        <input name="correos" type="hidden" value="'.$correo.'" />
                        <input name="zona_horarias" type="hidden" value="'.$zona_horaria.'" />
                        <input name="usuario_padres" type="hidden" value="'.$usuario_padre.'" />
                        <input name="tipos" type="hidden" value="'.$tipo.'" />
                        <input name="geocercass" type="hidden" value="'.$geocercas.'" />
                        <input name="aviso_accesos" type="hidden" value="'.$aviso_acceso.'" />
                        <input name="activas" type="hidden" value="'.$activa.'" />
			...
			if (isset($_POST['confirmacion'])) {
					...
					if($nombre_usuario==$_SESSION['MM_Username']) {$session_regenration_autentification='index.php';}
					else {$session_regenration_autentification='panel.php';}
...
odbc_exec($conexion, "UPDATE H_GEOCERCAS SET user_geo = '".$nombre_usuario."' WHERE user_geo='".$Boofer."'"); 
                 ...
                 <div id="funcion_opacidad">
					...
					<form action="'.$session_regenration_autentification.'" method="post">
						<input name="consulta" type="hidden" value="'.$consulta.'" />
						<input name="mostrar_registros" type="hidden" value="si" />';
						if (!isset($_POST['filtro'])) {
							echo'';
					...
					<br><br>
					<span style="display:block; width:100%;">Modificaci&oacute;n exitosa<br>Si cambio el nombre de usuario<br>valla a asignar veh&iacute;culo correspondiente</span>
					'; ?>
                 	</div>
                 </div>
