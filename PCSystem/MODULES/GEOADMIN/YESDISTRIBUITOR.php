...
if ($user_geocerca==$localFactoryUser) {
	$num_geocercas=$globalGG;
	$empresa=$localFactoryName;
	$usuario=$localFactoryUser;
	$laempresadelUsuario=$localFactoryName;
	$num_geocercas_por_usuario=$globalGG;
...
?>	    
			<table width="100%" border="0" bgcolor="#fff"  cellpadding="0" cellspacing="0" align="center" style="padding:10px;">
				<form action="panel.php" method="post">
				<tr height="30" bgcolor="#fff">
                    <td align="left" width="50%" valign="middle"><?php echo '<span style="float:left">N&uacute;mero de geo-cercas autorizadas para el usuario <span style="color:red; font-wight:bold;">"'.$usuario.'"</span> :&nbsp;</span>'; ?>
					<input type="text" name="num_geocercas_asignadas" value="<?php echo $num_geocercas_por_usuario ?>" style="width:50px; height:25px; float:left; text-align:center; color:red;" title="Escriba el numero de geo-cercas que desea asignar a este usuario" /></td>
					<input type="hidden" name="boofer_num_geo" value="<?php echo $num_geocercas_por_usuario ?>" />
					<input type="hidden" name="num_geocercas" value="<?php echo $num_geocercas ?>" />
					<input type="hidden" name="user_geocerca" value="<?php echo $usuario ?>" />
					<input type="hidden" name="empresa" value="<?php echo $laempresadelUsuario ?>" />
					<input type="hidden" name="consulta" value="admin_geocercas" />
					<input type="hidden" name="f1" value="f1" />
					<td align="right" width="50%"><input type="submit" value="guardar" style="background:#c2c5be; color:#fff; border:none; height:30px; cursor:pointer; padding-left:20px; padding-right:20px;"  /></td>
...
if (isset($_POST['nombre_geo'])){
	...
	if(!empty($_POST['nombre_geo'])){
		...
		if ($arr['counter']==0) {
						...
						$vertice_uno_cad_pura_lat=$prime_sum_vertex_lat;
						$vertice_dos_cad_pura_lat=$second_sum_vertex_lat;
						$vertice_tres_cad_pura_lat=$teer_rest_vertex_lat;

						$vertice_uno_cad_pura_lng=$prime_sum_vertex_lng;
						$vertice_dos_cad_pura_lng=$second_sum_vertex_lng;
						$vertice_tres_cad_pura_lng=$teer_rest_vertex_lng;

						$vertice_uno_cad_mapa_seguimiento=$prime_sum_vertex_lat.','.$prime_sum_vertex_lng;
						$vertice_dos_cad_mapa_seguimiento=$second_sum_vertex_lat.','.$second_sum_vertex_lng;
						$vertice_tres_cad_mapa_seguimiento=$teer_rest_vertex_lat.','.$teer_rest_vertex_lng;
						
						$ejecucion=odbc_exec($conexion,"INSERT INTO H_GEOCERCAS (nombre_geo,user_geo,empresa_geo,cad_ubica_geocerca,cadena_pura_lat,cadena_pura_lng,cad_mapa_seguimiento,estatus) VALUES ('".$nombre_geo."','".$user_geocerca."','".$empresa."', '{".$vertice_uno."},{".$vertice_dos."},{".$vertice_tres."},','".$vertice_uno_cad_pura_lat.",".$vertice_dos_cad_pura_lat.",".$vertice_tres_cad_pura_lat.",','".$vertice_uno_cad_pura_lng.",".$vertice_dos_cad_pura_lng.",".$vertice_tres_cad_pura_lng.",','[".$vertice_uno_cad_mapa_seguimiento."],[".$vertice_dos_cad_mapa_seguimiento."],[".$vertice_tres_cad_mapa_seguimiento."],','activa')");
