            ...
            <select class="cs-select cs-skin-underline" name="filtro">
					<option value="'.$mi_user_name.'">'.$mi_user_name.'</option>';
					 ...
					 //LISTAMOS TODOS LOS USUARIOS
							$consulta_todos_usuarios = odbc_exec ($conexion,"SELECT DISTINCT usuario_hijo FROM GG_MULTI_USUARIOS WHERE usuario_padre_secundario='".$mi_user_name."' ");
							while ($rows3 = odbc_fetch_object($consulta_todos_usuarios)) {
		
		    echo '<option value="'.$select_usuarios=$rows3->usuario_hijo.'">'.$select_usuarios=$rows3->usuario_hijo.'</option>';
